...
usando exactamente los mismos parámetros. El primero es usando el scheduler \texttt{RR2}:
...
\begin{itemize}
	...
	\item A nivel implementativo, \texttt{RR} es más fácil que \texttt{RR2}.
	\item Una de las ventajas de \texttt{RR2} es que no se paga el costo de cambio de cores, ya que cada tarea posee un único core en el que se ejecuta.
	\item Una desventaja de \texttt{RR2} es que puede, en algunas situaciones, dejar cores ociosos. \texttt{RR} también lo hace pero en mucha menor medida. En el caso de
	\texttt{RR} depende más que nada de la cantidad de tareas que quedan en relación al número de cores. En caso de \texttt{RR2}, pueden quedar muchas tareas asignadas a un solo core
	y ninguna al resto.
	\item Si hubiese a priori una política de balance de carga de los cores para \texttt{RR2}, estos estarían menos tiempo ociosos y mejoraría la performance del scheduler.
\end{itemize}